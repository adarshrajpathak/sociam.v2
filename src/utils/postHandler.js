...
async ()=>{
    try{
        const res = await axios.get('https://sociam-v2-backend.onrender.com/api/v1/posts/');
...
async(postId, {rejectWithValue})=>{
    try{
        const res = await axios.get(`https://sociam-v2-backend.onrender.com/api/v1/posts/${postId}`);
        return res.data;
    }catch(err){
        console.log(err);
...
async(postData, {getState, rejectWithValue})=>{
    try{
        ...
        const res = await axios.post('https://sociam-v2-backend.onrender.com/api/v1/posts/create',postData,{headers: 
    ...
    async (postData, { getState, rejectWithValue }) => {
      try {
        ...
        const res = await axios.get(
          `https://sociam-v2-backend.onrender.com/api/v1/likes/toggle/?id=${postData._id}&type=Post`,
          {
            headers: {
              Authorization: `Bearer ${authToken}`,
            },
    ...
    async (postData, { getState, rejectWithValue }) => {
      try {
        ...
        const res = await axios.post(
          "https://sociam-v2-backend.onrender.com/api/v1/bookmarks/toggle-bookmark",
          { postId: postData._id },
          {
            headers: {
              Authorization: `Bearer ${authToken}`,
...
async(postData, {getState, rejectWithValue})=>{
    try{
        const {authToken} = getState().auth;
        const res = await axios.post("https://sociam-v2-backend.onrender.com/api/v1/posts/edit",
          postData,
          {
            headers: {
              Authorization: `Bearer ${authToken}`,
...
async(postData, {getState, rejectWithValue})=>{
    try{
        ...
        const {authToken} = getState().auth;
        // console.log(authToken);
        const res = await axios.delete(`https://sociam-v2-backend.onrender.com/api/v1/posts/destroy/${postData._id}`, {headers: {authorization:`Bearer ${authToken}`}});
        // console.log(res.data);
        return res.data;
    }catch(err){
        console.log(err);
