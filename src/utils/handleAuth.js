...
const signupUser = createAsyncThunk("auth/signupUser",
    async (signupData, { rejectWithValue }) => {
        try {
            const res = await axios.post('https://sociam-v2-backend.onrender.com/api/v1/users/create', signupData);
...
const loginUser = createAsyncThunk("auth/loginUser",
    async (loginData, { rejectWithValue }) => {
        try {
            const {data} = await axios.post('https://sociam-v2-backend.onrender.com/api/v1/users/create-session', loginData);
